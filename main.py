    ...
    if choice == "Home":
        ...
        if st.checkbox("Login"):
            ...
            if result:
                ...
                op = ["None","Pose Identification","Profiles"]
                task = st.selectbox("Task", op)
                if task == "None":
                    st.write("Please select anyone from the list")

                elif task == "Pose Identification":
                    st.subheader("Pose Identification")
                    HtmlFile = open("pose_tensorflow.html", 'r', encoding='utf-8')
                    source_code = HtmlFile.read() 
                    #print(source_code)
                    components.html(source_code,height=1200,width=1200,scrolling=True)
